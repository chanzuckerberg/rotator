...
// for the given repo.
func (s *GitHubActionsSecretSink) Write(ctx context.Context, name string, value interface{}) error {
	switch writeVal := value.(type) {
	case string:
	...
	default:
		return errors.Errorf("Github Actions Sink doesn't support writing type %T", writeVal)
	}
}
